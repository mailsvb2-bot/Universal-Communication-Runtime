...
Schema v20 migrates v19 transactionally by adding only durable Event consumer state: subscription configuration, normalized Event-type filters, private committed/active cursor state, retry scheduling/action idempotency, replay generation/ID state, and dead letters. Canonical Event facts remain in the existing append-only `events` journal; `journal_seq` stays provider-private. Migration preserves every v19 row and creates no inferred subscription, cursor, replay, or dead letter.

Schema v21 adds the canonical Group aggregate, membership tombstones, bridge mappings and Group-change fingerprint journal without creating a second Conversation or Message owner.
Schema v22 adds canonical restart-safe CallSession, participant and signalling state while keeping Group/Conversation/Event owners separate.
Schema v23 migrates v22 transactionally by adding only `offline_group_changes` and `offline_group_messages` replication sidecars. The change sidecar stores restart-safe source enumeration semantics for newly committed canonical Group changes; the Message sidecar references the existing canonical Message row and stores only Group/source/generation enumeration metadata. Migration starts both sidecars empty and never infers source authorship, replication generation or Phase-26 replay evidence from historical v22 rows.
