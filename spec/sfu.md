...
The authenticated group-media header is versioned. Legacy **wire v1** remains readable byte-for-byte: audio is interpreted with no video source kind and legacy video is canonically interpreted as camera. Wire v1 can never claim screen-share authority because that distinction was not present in its authenticated header. **Wire v2** uses a distinct AEAD associated-data domain and carries an explicit authenticated video source kind: camera or screen-share. Relabelling a v2 screen-share frame as camera (or the reverse) invalidates the Device signature/AEAD binding before SFU fan-out. New endpoint group-media sealing emits the current v2 header; old v1 frames remain accepted by the new decoder under their narrower semantics.
