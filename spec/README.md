...
Phase 13 adds the first external-consumer boundary in `integration-api.md`; it reuses canonical Commands, Identity/Conversation/Message/Communication Intent owners and Service Principal admission rather than exposing Rust/storage internals. The first concrete gRPC adapter binds only `SubmitCommand` through that same ingress; generated Rust code is not normative protocol logic.
