...
`AuthorizedDurableRuntime` is the authorization-enforcing runtime boundary for every currently implemented tenant-scoped durable capability. Raw persistence traits remain internal storage/bootstrap capabilities and are not external authorization APIs. The runtime façade covers all current methods for permission grants, canonical Device lifecycle, trusted signing keys, recovery plans, command acceptance/outcomes, conversations, messages, delivery, sync, events, Service Principal administration, and Anti-Entropy.
