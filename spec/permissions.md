...
Every new **permission-authorized** tenant-scoped durable runtime method must add an explicit protocol-owned permission and authorization-enforcing façade method in the same change. Current architecture tests require complete method-for-method coverage; the current façade covers 43 externally callable tenant-scoped durable methods and the registry contains 35 unique permission IDs. The additional exact-operation Service Principal audit lookup reuses `ucr.audit.service_principal.read` because it is a narrower query over the same audit capability, not a new authority. This closes tenant-scoped authorization enforcement for the implemented durable runtime surface. Service Principal credential authentication feeds an authenticated `ScopedPrincipal` into the single-use quota/audit request evaluator and then this same authorization boundary. Canonical Device lifecycle administration now uses independent read/register/revoke permissions; device-bound credential/content delivery and remote-peer/transport authentication remain separate layers. Automatic quota consumption, audit append, and atomic key invalidation during Device revocation are internal security actions, not new externally callable authorization methods.
