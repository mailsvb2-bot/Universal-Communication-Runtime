...
The bounded reference config supports even dimensions from 16x16 through 1920x1080, 1–60 fps, and a fixed negotiated target bitrate from 64 kbit/s through 20 Mbit/s. The RFC-7742 minimum decode shape (320x240 at 20 fps unless otherwise signalled) is used as the Phase-21 reference default. The implementation configures OpenH264 Baseline and a realtime camera/screen usage mode, but UCR does **not** claim WebRTC/RFC-7742 conformance: VP8, SDP, RTP payload mapping, ICE and the WebRTC data plane are not Phase-21 claims.
...
The reference sender accepts one exact contiguous RGB8 frame, converts it to YUV420 through the codec library, encodes real H.264, and emits a bounded `EncodedVideoFrame`. The receiver first parses any H.264 SPS in safe Rust and requires its pixel dimensions to equal the negotiated dimensions before the native decoder sees that frame. The first decodable stream must include a successfully validated SPS. This safe-Rust SPS boundary is included in the bounded fuzz-smoke matrix. It then splits the encoded Annex-B/NAL stream, performs real H.264 decode, re-checks decoded dimensions, and returns owned RGB8 pixels. Encoded and decoded pixel payloads are redacted from `Debug` output.
