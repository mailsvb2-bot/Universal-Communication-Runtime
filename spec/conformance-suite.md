...
5. **webhook** — integrations use the canonical Event/Webhook subscription surface; executable conference evidence includes atomic `conference.started` / `conference.ended` lifecycle Events plus attendance `joined`, `left`, `reconnected`, and `media_ready` Events. Recording lifecycle webhook coverage remains required before Production integration certification;
