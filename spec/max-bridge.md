...
Only valid `message_created` text is normalized. Attachment-only or otherwise empty-text messages are unsupported by the Phase-34 Text-only manifest and are skipped while the provider marker is still allowed to advance. Provider message IDs become external event IDs, chat IDs become external conversation IDs, sender user IDs become external actor IDs, and provider millisecond timestamps remain millisecond timestamps. TenantScope and IntegrationId are supplied by Core to `BridgeRuntime::poll_events`; provider data cannot choose either.
