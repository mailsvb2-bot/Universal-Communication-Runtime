...
Trusted signing-key resolution is now an explicit runtime boundary. `TrustedSigningKeyResolver` resolves only an Active UCR-v1 Signing descriptor for an exact Tenant/Namespace scope, Device ID, optional canonical Identity binding, and key ID. A descriptor carried by a peer or referenced by a Message remains a claim: it must match independently resolved trust before handshake or Message verification proceeds. Resolution additionally requires the exact durable Device to be registered and `Active`; Message verification supplies `author_device.identity_id`, so rebinding a valid Device signature to another Identity fails as non-disclosing `NotTrusted`. Absence, revocation, non-Active Device state, and ordinary mismatch all fail closed.

The durable trusted-key lifecycle supports first provisioning, expected-current atomic rotation, irreversible key revocation, and exact retry idempotency. Revoked key IDs are never reactivated. SQLite v15 adds the separate canonical Device lifecycle owner rather than copying `DeviceLifecycleState` into trusted-key rows. Device revocation atomically revokes the current trusted signing key and blocks later provision/rotation/resolution until the Core-owned re-verification proof transition explicitly promotes the exact staged Device back to Active. Authorization of callers who mutate Device or key state remains the same explicit permissions/runtime responsibility.
