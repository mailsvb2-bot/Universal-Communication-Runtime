...
The local auth daemon and `ucr-auth-web` both refuse non-loopback plaintext binding. `ucr-auth-web` now provides the concrete HTTP Basic parser, `POST /oauth2/token`, authorization-server metadata and JWKS HTTP routes, while delegating token issuance to the loopback `MachineAuthService`. The shared `MachineBearerAdmissionRuntime` remains the semantic owner for verifying machine access tokens and standalone Permission Grant rechecks. For public API request admission, `MachineBearerRequestGate` composes that verification with the existing `ServicePrincipalRequestGate`, so Bearer requests use the same canonical request-rate quota and durable audit path as Service Credential requests instead of creating a second limiter or audit owner. The Universal Conference gRPC ingress now accepts exactly one authentication scheme per request: the existing Service Credential metadata or a standard `Authorization: Bearer <token>` value. Mixed schemes fail closed; all Universal Conference RPCs pass through the typed selector and machine-auth owns the canonical permission-to-OAuth-scope projection. Production `serve` and `serve-realtime` enable Bearer verification only when `UCR_MACHINE_TOKEN_VERIFICATION_JWKS_FILE` supplies a bounded public RFC 8037 JWKS together with the configured issuer/audience. The API process never loads the private signing seed; `serve-auth` remains the signing owner. External OAuth2 traffic must still terminate TLS at a trusted public edge before reaching the loopback gateway. `ucr-conference-web` is the thin loopback JSON adapter over Universal Conference gRPC. `ucr-https-edge` terminates TLS with an operator-supplied certificate and proxies the decrypted byte stream to exactly one loopback upstream. It does not authenticate callers or interpret conference requests. A local test proves one TLS handshake and an exact byte round-trip to loopback. That is not live public-deployment evidence: operators must still supply the certificate, DNS name and external reachability proof.
