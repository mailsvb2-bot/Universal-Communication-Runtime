...
The staged Device is always the plan-required `REVERIFICATION_REQUIRED` descriptor. Recovery staging never creates `ACTIVE` and never overwrites a differently bound/existing Device. A later, separate Core-owned `DeviceReverificationVerifier` must independently approve that exact staged Device before `ReverifiedDeviceActivationStore` may atomically promote only `REVERIFICATION_REQUIRED -> ACTIVE`. Ordinary registration or PermissionGrant administration is not a re-verification bypass, and a stale proof cannot resurrect a revoked Device. Any residual Active trusted signing key for a Device being registered/staged as non-Active is revoked in the same durable action; reopening SQLite rejects a non-Active Device row paired with an Active trusted key. This is especially important for v14→v15 databases whose historical trusted-key rows were intentionally preserved without inventing Device identity.
