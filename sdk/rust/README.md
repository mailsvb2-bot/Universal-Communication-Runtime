...
`UcrSdkClient` wraps the `IntegrationService`, `EventService`, `CallService`, `GroupService`, `DeviceService`, `SyncService`, `StoreForwardService`, `LocalTransportService` and `MeshService` RPCs, attaches authentication automatically, and returns canonical generated responses unchanged.
