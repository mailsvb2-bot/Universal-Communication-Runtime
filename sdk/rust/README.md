...
`UcrSdkClient` wraps the `IntegrationService`, `EventService`, `CallService`, `GroupService`, `DeviceService`, `SyncService`, `StoreForwardService` and `LocalTransportService` RPCs, attaches authentication automatically, and returns canonical generated responses unchanged.
