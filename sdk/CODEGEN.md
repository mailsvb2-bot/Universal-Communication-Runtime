...
The external-consumer SDK surface exposes generated clients for `IntegrationService`, `EventService`, `CallService`, `GroupService`, `DeviceService`, `SyncService`, `StoreForwardService`, `LocalTransportService`, `MeshService`, `RecoveryService` and the integration-facing `UniversalConferenceService`.
`UniversalConferenceService` is included because it uses the same Service Principal integration authentication. Participant-session services such as `RealtimeService` use join/session credentials and are not folded into the Service Principal helper. Other public protobuf services may also be generated as the contract grows; language helpers must not reinterpret their messages.
