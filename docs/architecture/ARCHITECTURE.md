...
Supported contract surfaces include protobuf/gRPC, HTTP where appropriate, event streams, local IPC and embedded APIs. They must express the same canonical semantics. The experimental `ucr-api-grpc` crate is the first concrete binding: all eleven checked-in `IntegrationService` RPCs are adapted and every one delegates to `IntegrationIngress`; generated protobuf Rust code and Tonic never become Core owners. Phase 13 is therefore complete at the local/reference Integration API layer. Production listener/TLS policy, HTTP, IPC and SDK packages remain separate work, and Phase 14 Event API has not started.
