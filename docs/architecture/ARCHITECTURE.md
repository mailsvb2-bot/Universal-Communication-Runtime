...
Supported contract surfaces include protobuf/gRPC, HTTP where appropriate, event streams, local IPC and embedded APIs. They must express the same canonical semantics. The experimental `ucr-api-grpc` crate is the first concrete binding: `SubmitCommand` plus `CreateIdentity`, `GetIdentity`, `LinkIdentity`, and `ResolveIdentityBinding` are currently adapted, and every one delegates to `IntegrationIngress`; generated protobuf Rust code and Tonic never become Core owners. The six remaining gRPC methods, production listener/TLS policy, HTTP, IPC and SDK packages remain incomplete.
