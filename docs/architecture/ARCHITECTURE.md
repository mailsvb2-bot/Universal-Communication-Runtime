...
Supported contract surfaces include protobuf/gRPC, HTTP where appropriate, event streams, local IPC and embedded APIs. They must express the same canonical semantics. The experimental `ucr-api-grpc` crate binds all eleven `IntegrationService` RPCs to `IntegrationIngress` and all eight `EventService` RPCs to `EventApiIngress`; generated protobuf Rust code and Tonic never become Core owners. Phase 13 is therefore complete at the local/reference Integration API layer. Phase 14 is complete at the local/reference Event API layer. Durable stream polling, reference webhook dispatch, retry, replay, backpressure, cursor idempotency and DLQ all use `EventSubscriptionStore` above the existing `EventJournalStore`. HTTP webhook networking, DNS discovery, production listener/service deployment, IPC and SDK packages remain separate work. Phase 15 now adds a Prepared `ucr-transport-internet` provider that reuses canonical Transport/Crypto/trust owners and accepts only public-IP TCP routes. Phase 24 supplies the separate Prepared Transport Orchestrator above all providers: it reuses canonical Intent/Policy/Endpoint/Transport owners and filters hard constraints before deterministic ranking. Phase 25 adds bounded sequential failover over that same plan and reuses a conservative `TransportProvider` acceptance-disposition boundary; a later route is attempted only after proven non-acceptance, while ambiguous acceptance stops fail closed. Phase 26 adds Prepared one-hop Offline Groups over canonical Group/Message/Sync/Device/Crypto owners: active `PeerPeer` SyncSessions select the Group Conversation, trusted established sessions prove exact Device peers, current Device/key trust is revalidated per operation, signed Messages are reverified, SQLite v23 persists only source-local replication sidecars, and reconciled remote records are never re-exported. Phase 27 adds Prepared sender-side Store-and-Forward as a bounded scheduling sidecar over canonical Intent/Message/Delivery/Policy/Endpoint/Transport owners: SQLite v24 persists jobs, leases and completion tombstones; no-route consumes no Delivery attempt; each provider invocation has one Delivery ID; proven retries receive a new Delivery ID; and ambiguous `InFlight` acceptance blocks automatic replay. Phase 28 adds Prepared signed-Group-Message Mesh over existing Group/Message/Sync/Device/Crypto owners: authenticated PeerPeer sessions remain the hop boundary, original authorship/signature is preserved, the loop-free Device path is capped at 8, and SQLite v25 stores only path metadata linked to existing offline Group Message rows. Phase 29 adds Prepared MLS-backed group-media SFU routing over canonical Group/Call/Device/authorization owners: OpenMLS owns standardized group epochs, SQLite v26 atomically binds security-sensitive Group changes to real MLS transitions, every encrypted frame is source-Device signed, fan-out recipients come from the current Call, and the SFU receives no plaintext/key material or durable Conference/Delivery authority. Phase 30 adds Prepared Conference coordination over those same owners: no second Conference store/roster is created, the shared bounded Call/Audio/Video/SFU ceiling is 1024, executable reference evidence reaches 1000 accepted participants, and recipient-owned ephemeral subscriptions bound selective encrypted fan-out to at most 32 source/media pairs per recipient. Production-scale load/SLA proof, sharded/federated SFU infrastructure, Relay/NAT traversal, discovery and multipath remain separate.
...
Phase 0 did not claim those implementations. The current repository now has Prepared Internet/local transports, Call signalling, Audio, Video, Phase-22 direct-call E2EE media protection, Phase-23 Adaptive Media policy, Phase-24 Transport Orchestrator planning, and Phase-25 bounded duplicate-safe Automatic Failover. Concrete Telegram/VK/MAX bridges, Relay/NAT traversal, discovery, arbitrary direct-message mesh, multipath, federation, general client SDK language bindings, automatic new-Device MLS admission/rekey policy, production-scale Conference/SFU load certification and production deployment remain later work; Phase 29 supplies the standardized MLS group-media/SFU foundation, Phase 30 supplies the Prepared Conference coordinator, and Phase 31 supplies the Prepared provider-agnostic Bridge SDK/trust boundary.


Phase 31 adds a Prepared Bridge SDK boundary without introducing provider-specific communication owners. `IntegrationId` remains canonical integration identity; `ucr-bridge` composes authorization, canonical Message policy/binding, live provider manifests and the durable Bridge registration/action metadata owners. SQLite v27 persists only registration compatibility/capability metadata and crash/idempotency action state/results, never provider plaintext. Concrete providers remain later phases.
