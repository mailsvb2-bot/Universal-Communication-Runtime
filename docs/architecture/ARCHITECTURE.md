...
Supported contract surfaces include protobuf/gRPC, HTTP where appropriate, event streams, local IPC and embedded APIs. They must express the same canonical semantics. The experimental `ucr-api-grpc` crate is the first concrete binding: only `SubmitCommand` is currently adapted, and it delegates to `IntegrationIngress`; generated protobuf Rust code and Tonic never become Core owners. Remaining gRPC methods, production listener/TLS policy, HTTP, IPC and SDK packages remain incomplete.
