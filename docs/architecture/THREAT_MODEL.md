...
The dedicated `ucr-security-tests` crate now has seven executable chaos scenarios for currently implemented local/reference boundaries: SQLite app restart with command deduplication, concurrent duplicate command ingress, Service Principal clock rollback with mandatory audit, local two-replica Anti-Entropy merge, old-client downgrade refusal, authenticated Message corruption, and revoked-Device restart. The exact evidence and nonclaims live in [`CHAOS_SCENARIOS.md`](./CHAOS_SCENARIOS.md). Deterministic mid-operation process-kill injection and end-to-end storage-full fault injection remain open. Network/DNS/Relay/SFU/peer-disappearance/packet-reorder/slow-consumer scenarios remain tied to future real transport/infrastructure boundaries rather than mocks.
...
- remaining chaos scenarios: deterministic process-kill and end-to-end storage-full fault injection for durable stores, plus network/DNS/Relay/SFU/peer-disappearance/transport-reorder/slow-consumer scenarios when those real boundaries exist;
