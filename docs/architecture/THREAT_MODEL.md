...
|---|---|---|---|
| MITM / forged peer | Device/Core/transport | authenticated handshake, key confirmation, peer identity binding | signed transcript, key confirmation, trusted signing-key lifecycle, and Active-Device gate implemented; remote peer Identity/transport integration pending |
...
| Malicious service account | External App/API | authenticated principal, least privilege, quotas, audit | credential auth + single-use quota gate + least-privilege runtime + append-only admission audit enforced |
| Stolen/revoked device | Device/Core/crypto | revoke, stop new key/content delivery, credential invalidation, audit | durable exact-scope lifecycle + trusted-key/authentication denial implemented; credential/content-delivery integration pending |
