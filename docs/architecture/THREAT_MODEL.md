...
- External App → SDK/API → UCR Core requires authenticated Service Principal context, tenant/namespace scope, permissions, quotas, and auditability. Direct database access is forbidden. The Phase-13 `IntegrationIngress` composes the existing single-use Service Principal admission proof with canonical durable Command, Identity, and Conversation operations. Identity, external-binding, and Conversation reads authorize before returning `NOT_FOUND`, so unauthenticated/unauthorized callers cannot probe resource existence. New external-binding keys require an existing exact Root Identity; opaque external entity bytes are not copied into audit operation references, and binding resolution audit identifies only the canonical `IntegrationId`. Conversation create/read reuses the existing provider-independent owner and binds generic audit only to canonical `ConversationId`; no provider Conversation mapping or direct storage path is introduced. Concrete network/IPC bindings remain separate.
