...
| Compromised SFU | `compromised_sfu_simulation_rejects_spoof_before_sink` | Phase-29 SFU → canonical Group/Call/Device/MLS/signing-key/permission authority → encrypted fan-out sink | spoofed source or ciphertext mutation cannot reach the SFU sink; the SFU API exposes ciphertext/routing metadata only | Implemented for Prepared MLS-backed group-media routing; Conference coordination remains Phase 30 |
