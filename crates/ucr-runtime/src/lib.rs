...
impl ProductionRuntime {
    ...
    pub fn open_existing(path: impl AsRef<Path>) -> Result<Self, String> {
        ...
        if !path.is_file() {
            return Err("production runtime requires an explicitly initialized database".to_owned());
        }
        let store = Arc::new(
            SqliteLocalStore::open(path).map_err(|error| format!("open durable store: {error:?}"))?,
        );
