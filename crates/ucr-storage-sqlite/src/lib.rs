...
pub use runtime_worker_store::{RuntimeWorkerLease, WEBHOOK_DELIVERY_WORKER_KIND};
...
const SQLITE_SCHEMA_V37: u32 = 37;
...
pub const SQLITE_SCHEMA_VERSION: u32 = 44;
...
fn initialize_or_validate_schema(connection: &mut Connection) -> Result<(), DurableStoreError> {
    ...
    if application_id == 0 && version == 0 {
        ...
        return initialize_schema_v23(connection);
    }
    if application_id != UCR_SQLITE_APPLICATION_ID {
        return Err(DurableStoreError::ForeignStore);
    }
    ...
    if version == SQLITE_SCHEMA_VERSION {
        return verify_schema_v44(connection);
    }
...
fn migrate_known_schema_to_current(
    ...
    while version < SQLITE_SCHEMA_VERSION {
        match version {
            ...
            SQLITE_SCHEMA_V37 => migrate_v37_to_v38(connection)?,
    ...
    verify_schema_v44(connection)
}

fn verify_schema_v44(connection: &Connection) -> Result<(), DurableStoreError> {
    verify_schema_v43(connection)?;
    service_control_store::verify_v44_objects(connection)
}

fn verify_schema_v43(connection: &Connection) -> Result<(), DurableStoreError> {
    verify_schema_v42(connection)?;
    runtime_worker_store::verify_v43_objects(connection)
}
...
fn verify_schema_v41(connection: &Connection) -> Result<(), DurableStoreError> {
    verify_schema_v37(connection)?;
    service_control_store::verify_v41_objects(connection)
}

fn verify_schema_v40(connection: &Connection) -> Result<(), DurableStoreError> {
    verify_schema_v37(connection)?;
    service_control_store::verify_v40_objects(connection)
}

fn verify_schema_v39(connection: &Connection) -> Result<(), DurableStoreError> {
    verify_schema_v37(connection)?;
    service_control_store::verify_v39_objects(connection)
}

fn verify_schema_v38(connection: &Connection) -> Result<(), DurableStoreError> {
    verify_schema_v37(connection)?;
    service_control_store::verify_v38_objects(connection)
}
...
fn initialize_schema_v23(connection: &mut Connection) -> Result<(), DurableStoreError> {
    ...
    service_control_store::create_v38_objects(&transaction)?;
...
fn migrate_v20_to_v21(connection: &mut Connection) -> Result<(), DurableStoreError> {
    ...
    group_store::create_v21_objects(&transaction)?;
    transaction
        .pragma_update(None, "user_version", SQLITE_SCHEMA_V21)
        .map_err(|error| map_sqlite_error(&error))?;
    transaction
        .commit()
        .map_err(|error| map_sqlite_error(&error))?;
...
fn migrate_v21_to_v22(connection: &mut Connection) -> Result<(), DurableStoreError> {
    group_store::verify_schema_v21(connection)?;
    let transaction = connection
        .transaction_with_behavior(TransactionBehavior::Immediate)
        .map_err(|error| map_sqlite_error(&error))?;
    call_store::create_v22_objects(&transaction)?;
    transaction
        .pragma_update(None, "user_version", SQLITE_SCHEMA_V22)
        .map_err(|error| map_sqlite_error(&error))?;
    transaction
        .commit()
        .map_err(|error| map_sqlite_error(&error))?;
...
fn migrate_v22_to_v23(connection: &mut Connection) -> Result<(), DurableStoreError> {
    call_store::verify_schema_v22(connection)?;
    let transaction = connection
        .transaction_with_behavior(TransactionBehavior::Immediate)
        .map_err(|error| map_sqlite_error(&error))?;
...
fn migrate_v26_to_v27(connection: &mut Connection) -> Result<(), DurableStoreError> {
    ...
    bridge_store::create_v27_objects(&transaction)?;
    transaction
        .pragma_update(None, "user_version", SQLITE_SCHEMA_V27)
        .map_err(|error| map_sqlite_error(&error))?;
    transaction
        .commit()
        .map_err(|error| map_sqlite_error(&error))?;
...
fn migrate_v27_to_v28(connection: &mut Connection) -> Result<(), DurableStoreError> {
    bridge_store::verify_schema_v27(connection)?;
    let transaction = connection
        .transaction_with_behavior(TransactionBehavior::Immediate)
        .map_err(|error| map_sqlite_error(&error))?;
...
fn migrate_v29_to_v30(connection: &mut Connection) -> Result<(), DurableStoreError> {
    ...
    personal_node_store::create_v30_objects(&transaction)?;
    transaction
        .pragma_update(None, "user_version", SQLITE_SCHEMA_V30)
        .map_err(|error| map_sqlite_error(&error))?;
    transaction
        .commit()
        .map_err(|error| map_sqlite_error(&error))?;
...
fn migrate_v30_to_v31(connection: &mut Connection) -> Result<(), DurableStoreError> {
    personal_node_store::verify_schema_v30(connection)?;
    let transaction = connection
        .transaction_with_behavior(TransactionBehavior::Immediate)
        .map_err(|error| map_sqlite_error(&error))?;
...
fn migrate_v37_to_v38(connection: &mut Connection) -> Result<(), DurableStoreError> {
    verify_schema_v37(connection)?;
    let transaction = connection
        .transaction_with_behavior(TransactionBehavior::Immediate)
        .map_err(|error| map_sqlite_error(&error))?;
    ...
    transaction
        .commit()
        .map_err(|error| map_sqlite_error(&error))?;
    verify_schema_v38(connection)
}

fn migrate_v38_to_v39(connection: &mut Connection) -> Result<(), DurableStoreError> {
    verify_schema_v38(connection)?;
    let transaction = connection
        .transaction_with_behavior(TransactionBehavior::Immediate)
        .map_err(|error| map_sqlite_error(&error))?;
    service_control_store::create_v39_objects(&transaction)?;
    transaction
        .pragma_update(None, "user_version", SQLITE_SCHEMA_V39)
        .map_err(|error| map_sqlite_error(&error))?;
    transaction
        .commit()
        .map_err(|error| map_sqlite_error(&error))?;
    verify_schema_v39(connection)
}

fn migrate_v39_to_v40(connection: &mut Connection) -> Result<(), DurableStoreError> {
    verify_schema_v39(connection)?;
    let transaction = connection
        .transaction_with_behavior(TransactionBehavior::Immediate)
        .map_err(|error| map_sqlite_error(&error))?;
    service_control_store::create_v40_objects(&transaction)?;
    transaction
        .pragma_update(None, "user_version", SQLITE_SCHEMA_V40)
        .map_err(|error| map_sqlite_error(&error))?;
    transaction
        .commit()
        .map_err(|error| map_sqlite_error(&error))?;
    verify_schema_v40(connection)
}

fn migrate_v40_to_v41(connection: &mut Connection) -> Result<(), DurableStoreError> {
    verify_schema_v40(connection)?;
    let transaction = connection
        .transaction_with_behavior(TransactionBehavior::Immediate)
        .map_err(|error| map_sqlite_error(&error))?;
...
fn migrate_v41_to_v42(connection: &mut Connection) -> Result<(), DurableStoreError> {
    ...
    service_control_store::create_v42_objects(&transaction)?;
    transaction
        .pragma_update(None, "user_version", SQLITE_SCHEMA_V42)
        .map_err(|error| map_sqlite_error(&error))?;
    transaction
        .commit()
        .map_err(|error| map_sqlite_error(&error))?;
...
fn migrate_v42_to_v43(connection: &mut Connection) -> Result<(), DurableStoreError> {
    verify_schema_v42(connection)?;
    let transaction = connection
        .transaction_with_behavior(TransactionBehavior::Immediate)
        .map_err(|error| map_sqlite_error(&error))?;
    runtime_worker_store::create_v43_objects(&transaction)?;
    transaction
        .pragma_update(None, "user_version", SQLITE_SCHEMA_V43)
        .map_err(|error| map_sqlite_error(&error))?;
    transaction
        .commit()
        .map_err(|error| map_sqlite_error(&error))?;
...
fn migrate_v43_to_v44(connection: &mut Connection) -> Result<(), DurableStoreError> {
    verify_schema_v43(connection)?;
    let transaction = connection
        .transaction_with_behavior(TransactionBehavior::Immediate)
        .map_err(|error| map_sqlite_error(&error))?;
...
const fn map_sqlite_error(error: &SqliteError) -> DurableStoreError {
    match error {
...
fn test_remove_v31_objects(connection: &Connection) -> Result<(), rusqlite::Error> {
    ...
    connection.execute_batch(
        "DROP TABLE IF EXISTS organization_managed_devices;
         ...
         DROP TABLE IF EXISTS organization_mode_profiles;",
    )
}

#[cfg(test)]
fn test_remove_v30_objects(connection: &Connection) -> Result<(), rusqlite::Error> {
    test_remove_v31_objects(connection)?;
    connection.execute_batch(
...
fn test_remove_v28_objects(connection: &Connection) -> Result<(), rusqlite::Error> {
    ...
    connection.execute_batch(
        "DROP TABLE IF EXISTS offline_group_bridge_changes;
         ...
         DROP INDEX IF EXISTS group_bridge_mappings_external_endpoint;",
    )
}

#[cfg(test)]
fn test_remove_v27_objects(connection: &Connection) -> Result<(), rusqlite::Error> {
    test_remove_v28_objects(connection)?;
    connection.execute_batch(
