...
use ucr_model::{
    ...
    ScopedPrincipal, ServiceAuditOutcome, ServiceAuditRecord, ServiceCredentialId,
    ServiceQuotaPolicy, TenantId, TenantScope,
};
use ucr_protocol::{
    MAX_SERVICE_AUDIT_READ_ITEMS, service_audit_hash, validate_service_audit_record,
        ...
        let mut statement = connection
            .prepare(
                "SELECT audit_id, credential_id, presented_tenant_id,
                 ...
                 FROM service_audit_records
                 WHERE presented_tenant_id=?1 AND presented_namespace_present=?2
                   AND presented_namespace_id=?3
                 ORDER BY audit_seq DESC LIMIT ?4",
            )
            .map_err(|error| map_sqlite_error(&error))?;
        let rows = statement
            .query_map(
    ...
    connection
        .query_row(
            "SELECT audit_id, credential_id, presented_tenant_id,
                    ...
                    resource_tenant_id, resource_namespace_present, resource_namespace_id,
                    outcome, occurred_at_unix_ms
             FROM service_audit_records WHERE audit_id=?1",
            params![audit_id.as_opaque().as_str()],
            decode_audit_row,
        )
        .optional()
        .map_err(|error| map_sqlite_error(&error))?
        .map(decode_audit_tuple)
        .transpose()
}

fn verify_audit_chain(connection: &Connection) -> Result<(), DurableStoreError> {
    let mut statement = connection
        .prepare(
            "SELECT audit_id, credential_id, presented_tenant_id,
                    presented_namespace_present, presented_namespace_id,
...
mod tests {
    ...
    use ucr_model::{
        ...
        PrincipalKind, PrincipalRef, ScopedPrincipal, ServiceAuditOutcome, ServiceAuditRecord,
        ServiceQuotaPolicy, TenantId, TenantScope,
    };
    use ucr_protocol::CONVERSATION_READ_PERMISSION;

    use super::SqliteLocalStore;
    use crate::{SQLITE_SCHEMA_V13, SQLITE_SCHEMA_VERSION, UCR_SQLITE_APPLICATION_ID};
