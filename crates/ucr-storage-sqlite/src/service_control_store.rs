...
const V40_OBJECTS_SQL: &str = "
ALTER TABLE service_resource_quota_policies
ADD COLUMN max_concurrent_publishers INTEGER
CHECK(max_concurrent_publishers IS NULL OR max_concurrent_publishers > 0);
";

const V41_OBJECTS_SQL: &str = "
ALTER TABLE service_resource_quota_policies
ADD COLUMN max_aggregate_bandwidth_bps INTEGER
...
pub(super) fn create_v40_objects(transaction: &Transaction<'_>) -> Result<(), DurableStoreError> {
    transaction
        .execute_batch(V40_OBJECTS_SQL)
        .map_err(|error| map_schema_change_error(&error))
}
...
pub(super) fn verify_v39_objects(connection: &Connection) -> Result<(), DurableStoreError> {
    ...
    verify_resource_quota_rows_v39(connection)
}

pub(super) fn verify_v40_objects(connection: &Connection) -> Result<(), DurableStoreError> {
    verify_table_columns(
        connection,
        "service_resource_quota_policies",
        &[
    ...
    verify_resource_quota_rows_v40(connection)
}

pub(super) fn verify_v41_objects(connection: &Connection) -> Result<(), DurableStoreError> {
    verify_table_columns(
        connection,
        "service_resource_quota_policies",
        &[
...
impl ServiceQuotaStore for SqliteLocalStore {
    ...
    fn set_service_resource_quota_policy(
        ...
        let max_concurrent_conferences = policy
            ...
            .transpose()
            .map_err(|_| DurableStoreError::InvalidRecord)?;
        let max_concurrent_publishers = policy
            .max_concurrent_publishers
            .map(i64::try_from)
            .transpose()
            .map_err(|_| DurableStoreError::InvalidRecord)?;
        let max_aggregate_bandwidth_bps = policy
...
impl ServiceAuditStore for SqliteLocalStore {
    ...
    fn service_audit_records(
        ...
        let mut records = rows
            .map(|row| {
                let (tuple, authentication_kind, operation_kind, operation_id) =
...
fn verify_resource_quota_rows_v39(connection: &Connection) -> Result<(), DurableStoreError> {
    let mut statement = connection
        .prepare(
            "SELECT tenant_id, namespace_present, namespace_id, principal_id,
                    max_concurrent_participants, max_concurrent_conferences
...
fn verify_resource_quota_rows_v40(connection: &Connection) -> Result<(), DurableStoreError> {
    let mut statement = connection
        .prepare(
            "SELECT tenant_id, namespace_present, namespace_id, principal_id,
                    max_concurrent_participants, max_concurrent_conferences,
...
fn decode_audit_tuple(row: AuditTuple) -> Result<ServiceAuditRecord, DurableStoreError> {
    ...
    let record = ServiceAuditRecord {
        ...
        authentication: ServiceAuthenticationRef::ServiceCredential(
            ServiceCredentialId::from_opaque(decode_id(&credential_id)?),
        ),
        presented_scope,
        subject,
        permission,
...
fn decode_audit_tuple_with_authentication(
    ...
    record.authentication = decode_audit_authentication(&stored_id, authentication_kind)?;
    validate_service_audit_record(&record).map_err(|_| DurableStoreError::Corrupt)?;
    Ok(record)
}
...
fn decode_audit_tuple_with_authentication_and_operation(
    ...
    let mut record = decode_audit_tuple_with_authentication(row, authentication_kind)?;
    record.operation = decode_audit_operation(operation_kind, operation_id)?;
    validate_service_audit_record(&record).map_err(|_| DurableStoreError::Corrupt)?;
    Ok(record)
}
...
fn verify_audit_chain_v44(connection: &Connection) -> Result<(), DurableStoreError> {
    let mut statement = connection
        .prepare(
            "SELECT r.audit_id, r.credential_id, r.presented_tenant_id,
                    r.presented_namespace_present, r.presented_namespace_id,
...
fn verify_audit_chain_for_schema(connection: &Connection) -> Result<(), DurableStoreError> {
    let schema_version: u32 = connection
        .pragma_query_value(None, "user_version", |row| row.get(0))
        .map_err(|error| map_sqlite_error(&error))?;
    if schema_version >= 44 {
        verify_audit_chain_v44(connection)
    } else if schema_version >= 17 {
...
mod tests {
    ...
    #[test]
    fn v39_to_v40_migration_preserves_existing_resource_quotas_and_adds_publisher_limit() {
        let db = TestDb::new();
        let subject = service("service-resource-v39-migration");
        {
            let _store = SqliteLocalStore::open(db.path()).expect("initialize current");
        }
        ...
        connection
            ...
            .expect("restore exact v39 resource quota shape");
        let namespace = super::namespace_storage_key(&subject.scope);
        connection
            .execute(
                "INSERT INTO service_resource_quota_policies (
            ...
            .expect("seed v39 resource quotas");
        connection
            .pragma_update(None, "user_version", SQLITE_SCHEMA_V39)
            .expect("set v39 version");
        drop(connection);

        let migrated = SqliteLocalStore::open(db.path()).expect("migrate v39 to v40");
        assert_eq!(migrated.schema_version(), Ok(SQLITE_SCHEMA_VERSION));
        assert_eq!(
            migrated
                .service_resource_quota_policy(&subject)
    ...
    #[test]
    fn v40_to_v41_migration_preserves_existing_resource_quotas_and_adds_bandwidth_limit() {
        let db = TestDb::new();
        let subject = service("service-resource-v40-migration");
        {
            let _store = SqliteLocalStore::open(db.path()).expect("initialize current");
        }
        ...
        connection
            .execute_batch(super::V39_OBJECTS_SQL)
            .expect("restore v39 resource quota table");
        connection
            .execute_batch(super::V40_OBJECTS_SQL)
            .expect("restore exact v40 resource quota shape");
        let namespace = super::namespace_storage_key(&subject.scope);
        connection
            .execute(
                "INSERT INTO service_resource_quota_policies (
            ...
            .expect("seed v40 resource quotas");
        connection
            .pragma_update(None, "user_version", SQLITE_SCHEMA_V40)
            .expect("set v40 version");
        drop(connection);

        let migrated = SqliteLocalStore::open(db.path()).expect("migrate v40 to v41");
        assert_eq!(migrated.schema_version(), Ok(SQLITE_SCHEMA_VERSION));
        assert_eq!(
            migrated
                .service_resource_quota_policy(&subject)
        ...
        connection
            .execute_batch(
                "DROP TRIGGER service_audit_operation_no_update;
                 ...
                 DROP TABLE IF EXISTS store_forward_jobs; DROP TABLE IF EXISTS store_forward_tombstones; DROP TABLE identities; DROP TABLE external_identity_bindings; DROP TABLE service_audit_operations;",
            )
